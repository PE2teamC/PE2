...
def Create_csv(file, Owner):
    ...
    for x in tqdm(file, desc='Saving xlsx files '):
        element = [Data_csv(x)[0], Data_csv(x)[1], Data_csv(x)[2], Data_csv(x)[3], Data_csv(x)[4], Data_csv(x)[5],
             Data_csv(x)[10], 'Version 1', Owner, Data_csv(x)[6], Data_csv(x)[7], Data_csv(x)[8], data_tm(x)[2],
             data_tm(x)[3], Data_csv(x)[9], data_tm(x)[0], data_tm(x)[1], data_IV(x)[0], data_IV(x)[1], data_IV(x)[2]]
        data.append(element)
        time.sleep(0.1)
    Excel_data = pd.DataFrame(np.array(data),
                      columns=["Lot", "Wafer", "Mask", "TestSite", "Name", "Date", "Script ID", "Script Version", "Script Owner",
                               "Operator", "Row", "Column", "Error Flag", "Error Description", "Analysis Wavelength(nm)",
